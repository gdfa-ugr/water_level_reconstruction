function [F]=EXTRACCION_VALOR_DE_F_MA_Q_MM_OD(Forzamiento,Forzamiento_3h,PC,M,pos,ini,SC, OF, var)
...
%          end
        ...
        pos_ini = find(Forzamiento.Q.t_q==t_sintetica(1));
        pos_fin = find(Forzamiento.Q.t_q==t_sintetica(end));
        t_ma = Forzamiento.Q.t_q(pos_ini:pos_fin);
        if strcmp(SC, 'SC_95_85')
            ma = Forzamiento.MA_95_85.ma(pos_ini:pos_fin);
        elseif strcmp(SC, 'SC_05_45')
            ma = Forzamiento.MA_05_45.ma(pos_ini:pos_fin);
        end
        ...
        % Comprobacion
        if pos_sintetica > length(WL_sintetica)
           pos_sintetica = length(WL_sintetica);
        end
        ...
        elseif strcmp(SC, 'SC_05_45')
            ...
            hold on
            %plot(Forzamiento_3h.MA_95_85.t_ma, Forzamiento_3h.MA_95_85.ma)
            plot(Forzamiento.MA_05_45.t_ma, Forzamiento.MA_05_45.ma, 'displayname', 'Forzamiento MA horario')
            plot(t_sintetica, WL_sintetica, 'displayname', 'WL Delft')
            plot(Forzamiento.MA_05_45.t_ma(pos(j)), Forzamiento.MA_05_45.ma(pos(j)), '.', 'markersize', 40, 'displayname', 'Pos MDA Forzamiento')
            plot(t_sintetica(pos_sintetica), WL_sintetica(pos_sintetica), '.', 'markersize', 40, 'displayname', 'Pos F')
            grid on
            legend('Show')
            xlim([t_sintetica(1)-0.5 t_sintetica(end)+0.5])
        ...
        F.MA_Q_MM_OD.eta(j)=WL_sintetica(pos_sintetica); %Solo MA,Q,MM
        F.MA_Q_MM_OD.Hs(j)=Hs; %Solo oleaje
        F.MA_Q_MM_OD.WL(j)=WL_sintetica(pos_sintetica)+Hs; %Total
        F.MA_Q_MM_OD.t_WL(j,:)=t_sintetica_vect(pos_sintetica,:);
